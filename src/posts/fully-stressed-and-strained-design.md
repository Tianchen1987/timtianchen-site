...
The method iteratively updates the array of areas A with respect to the stress utilization ratio of each bar, i.e. for a particular bar, its area increases if it experiences too much stress, and decreases if it does not. The iteration stops when each bar is fully stressed, i.e. its stress is at yield, hence the name.
